...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
    ...
    JOIN routes AS b ON
      (a.company = b.company AND a.num = b.num)
    JOIN stops AS stopa ON
      (a.stop_id = stopa.id)
    JOIN stops AS stopb ON
      (b.stop_id = stopb.id)
    WHERE
      stopa.name = 'Craiglockhart'
      AND
...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    ...
    JOIN routes AS start_routes ON
      start_routes.stop_id = start_stops.id
    JOIN routes AS s_to_t_routes ON
      s_to_t_routes.company = start_stops.company
      AND
      s_to_t_routes.num = start_stops.num
    WHERE
